...
EOF
  ...
  if [ $? -ne 0 ]; then
    echo "${CFAILURE}MySQL initialize failed! Check missing shared libraries: ldd ${mysql_install_dir}/bin/mysqld | grep 'not found' ${CEND}"
    kill -9 $$; exit 1;
  fi
  ...
  # RESET MASTER was removed in MySQL 8.4, replaced by RESET BINARY LOGS AND GTIDS
